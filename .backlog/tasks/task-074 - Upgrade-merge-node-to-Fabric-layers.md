...
2026-06-09: Fixed mixer-to-render low-resolution source regression. Full render source resolution now prefers uploaded full-quality URLs (`lastUploadUrl`) before preview URLs, while preview-quality resolution still uses `previewUrl`. Added regression coverage for direct render sources and V2 mixer layer compositions.

2026-06-11 follow-up: Fixed render-node display preview quality for mixer compositions. Root cause: useRenderNodePreview used preview-quality source URLs for medium zoom, so mixer layers could be composed from low-res previewUrl even when the render node was viewed close enough to need clarity. The display preview now uses full-quality sources for medium/high preview quality while retaining low-quality sources when zoomed far out. Added regression coverage in tests/use-render-node-preview.test.ts. Verification: npm run test -- tests/use-render-node-preview.test.ts tests/lib/canvas-render-preview.test.ts tests/use-pipeline-preview.test.ts tests/instagram-post-mockup-node.test.ts; npm run lint -- components/canvas/nodes/use-render-node-preview.ts tests/use-render-node-preview.test.ts components/canvas/nodes/instagram-post-mockup-node.tsx tests/instagram-post-mockup-node.test.ts; git diff --check on same files. Browser reload of affected canvas produced 0 warn/error logs.

2026-06-11 follow-up implementation started: approved plan is to make Mixer canvas preview zoom-aware only. Steps: (1) add tests for Mixer medium/full and low/preview source selection plus Fabric retina scale caps, (2) pass previewQuality into MixerFabricEditor, (3) compute displaySourceQuality as preview only for low quality and full for medium/high, (4) apply capped Fabric retina/backing-store scale for Canvas-only preview, (5) update canvas docs and verify targeted tests/lint/browser smoke.

2026-06-11 implementation result: Mixer/Merge canvas preview is now zoom-aware for canvas display only. Medium/high zoom use full-quality source URLs for composition and a capped Fabric backing-store scale based on preview quality and DPR; low zoom continues using preview URLs with lighter scaling. Added regression coverage for URL choice and Fabric retina scale. Targeted Vitest matrix and lint passed; full suite still has an unrelated pre-existing canvas-flow-reconciliation-helpers pinned-size expectation failure.

2026-06-11 follow-up started: downstream Bildausgabe/Render node connected to Mixer still displays low-resolution output. Debug plan: trace render-node preview resolution after Mixer, add a failing regression around Mixer input source quality/preview display, implement minimal canvas-only propagation fix, then run targeted tests/lint/browser smoke.

2026-06-11 follow-up implementation result: downstream Render/Bildausgabe previews for Mixer compositions now request a larger canvas-only preview width: low 360, medium 1280, high 1920. Added an explicit previewWidth override to usePipelinePreview and wired it only when useRenderNodePreview displays sourceComposition.kind=mixer. Normal media preview buckets and final render/upload output remain unchanged. Targeted tests/lint passed; full pnpm test still fails only on the pre-existing canvas-flow-reconciliation-helpers pinned-size expectation.

2026-06-11 second follow-up started: user verified downstream Render/Bildausgabe output itself is 240x320 px after Mixer, not merely a soft canvas preview. Debug scope shifts to full render/export sizing for sourceComposition.kind=mixer while preserving canvas-only preview behavior and avoiding unrelated worktree changes.

2026-06-11 second follow-up fixed downstream Render/Bildausgabe low-resolution mixer exports. Full render loading now treats a same-aspect persisted mixer stage that is smaller than the base layer bitmap as stale and renders at the base bitmap size instead. Render pipeline hashes now include a mixer render sizing version so existing 240x320 exports are invalidated; browser smoke after reload shows Bildausgabe IDLE with "Pipeline geaendert. Bitte erneut rendern." and no console warnings/errors. Targeted tests and lint passed.

2026-06-11 verification update: pnpm test still has one unrelated pre-existing failure in components/canvas/__tests__/canvas-flow-reconciliation-helpers.test.ts (keeps pinned local node size until convex catches up). The targeted mixer/render/source-loader suites passed.
<!-- SECTION:NOTES:END -->
