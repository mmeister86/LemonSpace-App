/**
 * Onboarding note:
 * Convex backend module for ai text pipeline. Keep auth checks, ownership validation, and idempotency close to the mutation/query that touches user data.
 */
  ...
  return register({
    ...
    handler: async (
      ...
      try {
        ...
        await releasePublicReservationBestEffort(ctx, reservationId, "ai");
        await decrementConcurrencyIfNeeded(ctx, usageIncremented, canvas.ownerId);
        throw error;
      }
